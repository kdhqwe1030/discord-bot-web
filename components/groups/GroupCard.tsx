"use client";

import UserProfile from "@/components/users/UserProfile";
import { useUserStore } from "@/stores/userStore";
import { FaDiscord } from "react-icons/fa";

const GroupCard = () => {
  const { data } = useUserStore();
  const avatarUrl = data?.user?.user_metadata?.avatar_url || "";

  return (
    <div className="w-full bg-sub2 rounded-2xl p-6 shadow-lg shadow-black/40 text-white h-64">
      <div className="flex items-start justify-between">
        <div className="space-y-1">
          <h2 className="text-xl font-semibold leading-tight">그룹이름</h2>
          <div className="flex items-center gap-2 text-sm text-gray-300">
            <FaDiscord className="text-discord" />
            <span>디스코드 방이름</span>
          </div>
        </div>

        <div className="inline-flex items-center rounded-full bg-sub3/70 px-3 py-1 text-xs text-gray-300 border border-white/5">
          <span className="mr-1 font-medium">5</span>
          <span>members</span>
        </div>
      </div>

      <div className="mt-5 flex gap-4">
        <div className="flex-1 rounded-xl bg-sub3 px-4 py-3 flex flex-col justify-between">
          <span className="text-[11px] uppercase tracking-[0.12em] text-gray-400">
            Win Rate
          </span>
          <span className="mt-2 text-2xl font-semibold text-green-400">
            58.4%
          </span>
        </div>

        <div className="flex-1 rounded-xl bg-sub3 px-4 py-3 flex flex-col justify-between">
          <span className="text-[11px] uppercase tracking-[0.12em] text-gray-400">
            Matches
          </span>
          <span className="mt-2 text-2xl font-semibold">142</span>
        </div>
      </div>

      <div className="mt-4 flex items-center -space-x-3">
        {/* 실제 아바타 */}
        <UserProfile imgUrl={avatarUrl} />
        <UserProfile imgUrl={avatarUrl} />
        <UserProfile imgUrl={avatarUrl} />
        <UserProfile imgUrl={avatarUrl} />
        <span className="flex h-8 px-3 items-center justify-center rounded-full bg-sub3 text-xs font-medium">
          +2
        </span>
      </div>
    </div>
  );
};

export default GroupCard;
